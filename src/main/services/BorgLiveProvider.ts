...
export class BorgLiveProvider implements IBorgProvider {
  ...
  async transitionPhase(sessionId: string, completedPhaseId: number, nextPhaseId?: number): Promise<void> {
    logger.info(`Transitioning phase for session ${sessionId}: completed=${completedPhaseId}, next=${nextPhaseId}`, LOG_CONTEXT);
    ...
    if (handoff.maestro) {
      ...
      logger.warn(`Cannot transition phase: Maestro metadata missing in handoff for session ${sessionId}`, LOG_CONTEXT);
    }
  }
