...
- Version bumped dynamically to `v0.15.28` across all core files.

## Next Steps
- Implement Context Compaction: Per the roadmap, extend the `AgentDetector` to support token-aware summarization and context squashing.
- Hydrate the built-in agents by executing real Subprocess commands connected to the `ConfigManager` API keys.
